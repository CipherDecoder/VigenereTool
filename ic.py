...
from const import RU_IC, ALPHABET


def _ic(letter_counts):
    numerator = sum([letter_counts[l]*(letter_counts[l]-1) for l in ALPHABET])
...
def find_key_length(ciphertext, max_key_len):
    ...
    for candidate_length in range(1, max_key_len + 1):
        groups = processing.get_blocks(text=ciphertext, size=candidate_length)
        ...
        if RU_IC-delta_bar_ic < min_diff:
            min_diff = RU_IC-delta_bar_ic
